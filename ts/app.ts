...
const multyEventsOnElements = function (
  elements: any,
  type: string,
  callback: EventListener
) {
  for (const element of elements) element.addEventListener(type, callback);
};
...
searchField.addEventListener("input", function () {
  ...
  if (searchField.value) {
    searchTimeout = setTimeout(() => {
      fetchData(url.geocoding(searchField.value), function (loc: any) {
        searchResult.classList.remove("hidden");
        ...
        for (const { name, lat, lon, country, state } of loc) {
          ...
          searchResult
            ...
            ?.appendChild(searchedItem);
          items.push(searchedItem.querySelector("[data-search-toggler]"));
        }

        multyEventsOnElements(items, "click", function (e: Event) {
          searchResult.classList.add("hidden");
          searchField.value = "";
        });
      });
...
const currLocBtn = document.querySelector(
  "[data-current-location-btn]"
) as HTMLButtonElement;
const error = document.querySelector("[data-error]") as HTMLElement;

export const updateWeather: Function = function (
  lat: number,
  lon: number
): void {
  const currentWeatherDiv = document.querySelector(
    "[data-today-weather]"
  ) as HTMLElement;
  const incomingWeatherDiv = document.querySelector(
    "[data-incoming-weather]"
  ) as HTMLElement;
  const todayInfoDiv = document.querySelector(
    "[data-today-info]"
  ) as HTMLElement;
  const todayAtDiv = document.querySelector("[data-today-at]") as HTMLElement;
  ...
  fetchData(
    !url.currentWeather(lat, lon).includes("lat")
      ? url.currentWeather(lat, lon).replace("?", "?lat=").replace("&", "&lon=")
      : url.currentWeather(lat, lon),
    function (currentWeather: any) {
      const {
        weather: [{ description, icon }],
        dt: dateInSec,
        sys: { sunrise: sunriseInSec, sunset: sunsetInSec },
    ...
    `;
      ...
      fetchData(
        !url.reverseGeo(lat, lon).includes("lat")
          ? url.reverseGeo(lat, lon).replace("?", "?lat=").replace("&", "&lon=")
          : url.reverseGeo(lat, lon),
        function ([{ name, country }]: any) {
          card.querySelector(
            "[data-location]"
      ...
      fetchData(
        !url.airPollution(lat, lon).includes("lat")
          ? url
              .airPollution(lat, lon)
              .replace("?", "?lat=")
              .replace("&", "&lon=")
          : url.airPollution(lat, lon),
        function (airPollution: any) {
          const [
            {
              main: { aqi },
      ...
      <h2 class="card-title">Todays Info</h2>
          ...
          <div class="air card-sm">
            <div class="air-quality">
              ...
              <p class="card-desc-sm colored colored-${aqi}" title="${
            module.aqi[aqi].message
          }">${module.aqi[aqi].level}</p>
            </div>
            <div class="air-other">
              <div class="icon-div">
            ...
            <div class="sunrise-other">
              <div class="sunrise-box">
                ...
                <div class="sunrise-div-col">
                  ...
                  <p class="card-num">${module.getTime(
                    sunriseInSec,
                    timezone
                  )}</p>
                </div>
          ...
          <div class="humidity card-sm">
            ...
            <div class="humidity-other">
              ...
              <div class="icon-div">
                <p class="card-num">${Number(
                  humidity
                )}<span class="today-info-span">%</span></p>
              </div>
            </div>
          </div>
          ...
          <div class="pressure card-sm">
            ...
            <div class="pressure-other">
              ...
              <div class="icon-div">
                <p class="card-num">${Number(
                  pressure
                )}<span class="today-info-span" >hPa</span></p>
              </div>
            </div>
          </div>
          ...
          <div class="visibility card-sm">
            ...
            <div class="visibility-other">
              ...
              <div class="icon-div">
                <p class="card-num">${Number(
                  visibility / 1000
                )}<span class="today-info-span">km</span></p>
              </div>
            </div>
          </div>
          ...
          <div class="feels-like card-sm"><div class="humidity-heading">
            ...
            <div class="feels-like-other">
              ...
              <div class="icon-div">
                <p class="card-num">${parseInt(
                  feels_like
                )}<span class="today-info-span">&deg;c</span></p>
              </div>
            </div></div>
      ...
      fetchData(
        !url.incomingWeather(lat, lon).includes("lat")
          ? url
              .incomingWeather(lat, lon)
              .replace("?", "?lat=")
              .replace("&", "&lon=")
          : url.incomingWeather(lat, lon),
      ...
      `;
        ...
        <p class="card-desc-sm">${module.getHours(dateInSec, timezone)}</p>
            <ion-icon name="navigate-outline" class="card-icon-sm" style="transform: rotate(${
              windDirection + 140
            }deg)"></ion-icon>
            <p class="card-desc-sm">
              ${parseInt(
                module.mpsToKmh(windSpeed)
              )} <span class="today-at-span">km/h</span>
            </p>
        `;
    ...
    `;
      ...
      <div class="icon-container">
                ...
                <p class="card-desc-sm">${date.getDate()} ${
              module.months[date.getMonth()]
            }</p>
      ...
      `;
            ...
            incomingWeatherDiv
              .querySelector("[data-incoming-div]")
              ?.appendChild(li);
          }
        }
      );
