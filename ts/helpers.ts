...
export const getDate = function (dateInSec: number, timezone: number) {
...
export const getTime = function (timeInSec: number, timezone: number) {
...
export const getHours = function (timeInSec: number, timezone: number) {
  const date = new Date((timeInSec + timezone) * 1000);
  const hours = date.getUTCHours();
  const timePeriod = hours >= 12 ? "PM" : "AM";
...
export const mpsToKmh: Function = function (mps: number) {
  const mph = mps * 3600;
  return mph / 1000;
};
